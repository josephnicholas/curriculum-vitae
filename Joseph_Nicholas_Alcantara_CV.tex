...
\begin{cvsection}{Employment History}
	\begin{cvsubsection}{Software Engineer}{Archax, Philippines}{February 2021 - Present}
		\color{darkgray}
		\begin{itemize}
			\item Implemented Risk Management for Archax OMS.
			...
			\item Implemented admin API functionalities in the Crypto Matching Engine Library to interface between the Crypto Matching Engine and OMS.
			\item Implemented a Python test client that will send messages on behalf of the OMS or the admin application.
			\item Implemented round-robin connection/reconnection logic for preparation for Hot-Warm High-Availability.
			\item Created a cross-project utility library. Extracted and refactored common helper functions from across projects.
			\item Implemented a message version check upon logon which validates if both client and the Crypto Matching Engine messages are aligned.
			\item Implemented a performance tool for the Crypto Matching Engine which will send a payload of orders and calculate acceptable latency metrics.
			\item Implemented reconnect and timeout logic for non-blocking client connections in the internal library which is consumed by OMS and the Crypto Matching Engine .
			\item Implemented buffer(executions, orders, etc.) processing and storage by session in the internal library which is consumed by OMS and the Crypto Matching Engine.
			\item Designed a Metric service that will query all business metrics for ACE-OMS rebalancing
			\item Designed ACE-OMS rebalancing flow
			\item Implemented a ace-fix client in Golang to test inbound and outbound messages from the exchange
		\end{itemize}
	\end{cvsubsection}
...
\begin{cvsection}{Additional Experience and Awards}
	...
	\begin{cvsubsection}{}{}{}
		\begin{itemize}
			\item \textbf{1st Prize, 1st Engineering Robotics competition:}  Maze Traversal and Line Following category
		\end{itemize}
	\end{cvsubsection}
\end{cvsection}
...
\begin{cvsection}{Programming languages and Technologies}
	\color{darkgray}
	\begin{cvsubsection}{}{}{}
		\textbf{Language}(arranged according to proficiency, last 3 ones are basic knowledge)
		\begin{itemize}
			\item C++11/14/17, C, Kotlin, Python, JavaScript, TypeScript, Solidity, SQL 
		\end{itemize}
		\textbf{Paradigms}
		\begin{itemize}
		...
		\textbf{Libraries}
		\begin{itemize}
			\item C++-STL, Boost, Smart Contracts(Corda and Ethereum), Abseil-C++, Radia, Protobuf, Unix Socket Library
		\end{itemize}
		\textbf{Database}
		\begin{itemize}
